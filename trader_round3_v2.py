...
class Trader:
    ...
    # HYDROGEL is the main alpha. VELVET_EXTRACT and mid-strike vouchers
    # were dragging PnL, so we only keep vouchers that were consistently positive
    # or essentially free far-wing liquidity collection.
    ACTIVE_VOUCHERS = {
        "VEV_4000",  # consistently strong deep-ITM parity market making
        "VEV_5400",  # consistently positive in your 3-day backtest
        "VEV_5500",  # consistently positive in your 3-day backtest
        "VEV_6000",  # tiny but positive far-wing edge
        "VEV_6500",  # tiny but positive far-wing edge
    }
    ...
    # Round 3 final simulation starts at 5 days to expiry.
    TTE_DAYS = 5.0
    ...
    def run(self, state: TradingState):
        ...
        # Disabled as a standalone product after your backtest showed it was a
        # consistent drag (-3.7k total across days 0/1/2). We still read its mid
        # as the underlying for voucher pricing.
        ...
        if self.VELVET in mids:
            ...
            for product, strike in self.STRIKES.items():
                ...
                fair = model_fair - 0.025 * position

                option_max_take = self.OPTION_TAKE_SIZE
                option_mm_size = self.OPTION_MM_SIZE
                if product == "VEV_4000":
                    option_max_take = 80
                    option_mm_size = 55
                ...
                self.trade_option_product(
                    ...
                    take_edge=take_edge,
                    mm_width=mm_width,
                    max_take=option_max_take,
                    mm_size=option_mm_size,
                )
    ...
    def static_smile_iv(self, m: float) -> float:
        # Historical Round 3 data showed central vol around 22-24%.
        # Wings require higher IV to match the quoted 0/1 premium region.
        if m > 0.75:
            iv = 0.222 + 0.155 * m * m
        elif m < -1.0:
            iv = 0.225 + 0.010 * m * m
        else:
            iv = 0.228 + 0.020 * m + 0.035 * m * m
        return max(0.07, min(0.90, iv))
